...
public class GTranslateV2Module extends ToggleableModule {
  public static GTranslateV2Module INSTANCE;
  ...
  public GTranslateV2Module() {
    super("GTranslateV2Module", "Translates using google translate", ModuleCategory.CLIENT);
  ...
  public boolean onChat(String message) {
    if (!this.translateSending.getValue() || secondcancel) return false;
    try {
      new Thread(() -> {
        String text = "";
        try {
          text = translate(message, sendLang.getValue().name());
        } catch (Exception ex) {
